...
Using a plain Statamic form tag such as `{{ form:contact }}` so Statamic renders the form from its form configuration; the extension inserts the fuller example only when you accept the completion example shortcut with `Cmd+Enter` or `Ctrl+Enter`.
_Avoid_: Automatic form syntax, default form

**Manual form partials**:
Editable form partials generated by the extension with a form completion such as `{{ form:contact:manual }}`.
_Avoid_: Custom form, generated form

**Generated search partial**:
A reusable search partial created by the extension from `{{ search:generate }}` that renders and updates collection results.
_Avoid_: Search generator, AJAX search partial

**Editing helpers**:
Small extension behaviors that reduce repetitive Antlers editing, such as completion examples, paired tags, linked editing, and partial navigation.
_Avoid_: Magic, shortcuts only
